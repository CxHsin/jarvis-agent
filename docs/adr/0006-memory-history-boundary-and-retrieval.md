...
Memory retrieval uses Query Rewrite followed by parallel BM25 and vector search, RRF rank fusion, validity filtering, and deduplication. HyDE runs only when the first pass yields fewer than three valid facts or its best relevance is below the configured threshold. Default retrieval returns current facts; temporal queries may include invalidated history. Dynamic results are limited to eight facts and 1,000 tokens.
