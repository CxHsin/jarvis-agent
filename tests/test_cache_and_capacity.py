...
class CapacityTests(unittest.TestCase):
    ...
    def test_reserve_line_drives_the_trigger_and_the_send_limit(self):
        manager = ContextManager(self.config(context_window_tokens=10000, context_reserve_tokens=1000))
        # One reserve: compression starts exactly where sending would be refused.
        self.assertEqual(manager.input_budget(), 9000)
        self.assertFalse(manager.should_compress(9000))
        self.assertTrue(manager.should_compress(9001))

    def test_reserve_line_respects_the_model_input_cap(self):
        budget = ContextBudget.from_config(make_config(context_window_tokens=100000, context_reserve_tokens=16384,
                                                       model_max_input_tokens=50000, max_output_tokens=32768))
        self.assertEqual(budget.reserve, 16384)
        self.assertEqual(budget.input_limit, 50000)
        self.assertEqual(budget.trigger, 50000)
        self.assertFalse(budget.over_budget(50000))
        self.assertTrue(budget.over_budget(50001))

    def test_output_limit_follows_the_remaining_window(self):
        budget = ContextBudget.from_config(make_config(context_window_tokens=100000, context_reserve_tokens=16384,
    ...
    def test_output_limit_clamps_on_the_wire_and_still_refuses_oversize(self):
    ...
    def test_reserve_validation_uses_one_limit(self):
        with self.assertRaises(ConfigurationError):
