...
class TaskHistory:
    def __init__(self, directory: Path, session_id: str, count: int):
    ...
    def _apply(self, event):
        ...
        if kind == "task":
            ...
            self.tasks.append(dict(event, messages=[], status="active"))
    ...
    def record(self, record):
        ...
        event = dict(record, occurred_at=now, recorded_at=now)
        if event["type"] == "task":
        ...
        if event["type"] == "task":
            ...
            with history.open("a", encoding="utf-8") as handle:
                handle.write(f'- occurred_at={now} recorded_at={now} '
                             f'task={event["task_id"]} source={self.path.name}: {query}\n')
                handle.flush()
                os.fsync(handle.fileno())
